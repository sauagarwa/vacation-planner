...
def _coerce_list(value: str) -> list[str]:
    ...
    try:
        ...
        pass
    # Extract JSON array from fenced or inline content.
...
def google_flights_search(
    ...
    # Prefer first and last cities (start/end), skip mid-journey attractions.
    selected = []
    if destinations:
    ...
    for dest in selected[:max_destinations]:
        ...
        if response.status_code >= 400:
            results.append(
                f"- SerpApi error {response.status_code} for {destination_code}: {response.text}"
            )
            continue
        response.raise_for_status()
        data = response.json()
